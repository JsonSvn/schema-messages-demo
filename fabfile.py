...
def get_node():
	return check_for('node', 'You need to install Node.js')

def get_npm():
	return check_for('npm', 'You need to install Node.js and npm')
...
def httpd_config():
	...
	rsync_project(
		...
		os.path.join(BASEDIR, "server-configs", "schema-sockets-demo.conf")
	)
...
def game_restart():
	...
	require('root', provided_by=PROVIDERS)
	run('supervisorctl restart schemasocketsdemo')
